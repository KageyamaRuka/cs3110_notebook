...
module MakeListDictionary
  = functor (K : Comparable) -> functor (V : Formattable) ->
struct
  module Key = K
  module Value = V
  type key = K.t
...
module MakeSetOfDictionary
  = functor (C : Comparable) -> functor (DM:DictionaryMaker) ->
struct
  module Elt = C
  type elt = Elt.t
